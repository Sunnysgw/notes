...
- cms
  ...
  cms这种动态回收的方式会有问题，有漏标或者多标的情况，针对**漏标**，有两种方式来处理：
  
  - 增量更新
  
    记录增加的新的引用，之后再做扫描，重新更新
  
  - 原始快照
  
    记录引用关系变化的对象，直接标记为黑色，不被当作垃圾

**记忆集和卡表**

对于跨代引用的场景，会把老年代中对年轻代有引用的对象放到记忆集中，卡表是记忆集的具体实现。卡表是一个数组，数组中的每个元素对应一块老年代中的内存空间，如果某个内存空间中的对象存在跨代引用，即做标记。之后做young gc的时候，会去扫描这些做了标记的元素对应的地址。

### 4.3 G1

jdk9中，G1已经成为了默认的垃圾收集器。如果使用G1收集器，堆的结构就没有那么清晰了，年轻代、老年代没有分的那么清晰。

堆空间划分成多个大小相同的region，默认是2048个，老年代、年轻代不再会有物理上的界限。每个region会有这几个角色：eden、survivor、old、humongous（专门放大对象），角色可能会动态变化。这里eden s0 s1的比例默认还会是8：1：1。

G1和前面讲的收集器关于内存分配的不同点是关于大对象的分配，如果一个对象的大小超过了region的50%，则会被当做大对象，被放到humougous角色的区域中。
...
- 筛选回收
  ...
  会stw，对region的回收价值（？）和成本做排序，之后基于用户给定的期望GC停顿时间制定回收计划。使用标记复制算法做回收。
...
- 老年代占用空间超过整个堆内存的比例超过多少，执行mixedgc
- region中的存活对象低于某个阈值，即开始回收
- 筛选回收的次数

对于大内存，且使用率高的堆，paral new这种垃圾收集效率不会太高，更需要把stw时间分散就，g1把本来整块eden的垃圾回收打散成几块，提升用户体验。

> 这里感觉核心的理念就是并发+分散，从serial 到 paral 到 cms 到 g1，就是从单线程 到多线程 到用户线程、垃圾回收线程并发执行，到动态设置回收时间。
...
- stw时间足够短
- 对吞吐量影响小
- stw不会随着内存增加而降低，意思就是几十G是10ms，同时到TB级别的stw也是10ms
