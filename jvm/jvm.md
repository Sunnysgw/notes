...
- 内存分配
  ...
  - cas，保证同时只有一个线程竞争成功
  - tlab，thread local allocation buffer，1.8之后，默认采用这种方式，即在eden中给每个线程分配一块空间，不同线程操作自己分配到的那块，避免冲突。

- 初始化

- 设置对象头

  堆中对象分配堆空间主要是由 对象头、实例数据、填充部分（保证对象大小是8字节的整数倍） 构成。对象头记录了一个对象在jvm中通用的一些属性，包括类型指针、age、加锁情况等。