...
- b树
  ...
  ![image-20220928090224608](../image-20220928090224608.png)

- b+树

  针对b树做了一些优化

  ![image-20220928090303172](..\image-20220928090303172.png)
...
![截屏2022-09-30 11.32.55](../explain1.png)
...
- join
  ...
  ![image-20221001110104191](..\image-20221001110104191.png)

  以上扫描算法是**嵌套循环连接(Nested-Loop Join NLJ)算法**。

  这里前提是扫描大表用了索引，如果没有用索引，大表就要做全表了，扫描记录如下：

  ![image-20221001110516779](..\image-20221001110516779.png)
...
- 间隙锁
  ...
  ![image-20221003095508401](../image-20221003095508401.png)
...
mysql的数据表，使用innodb引擎的时候，提供了一些隔离级别:
...
其中，read-commited和repeatable-read两种隔离级别是基于innodb引擎中的多版本并发控制(mvcc)机制实现的，机制的诉求是，控制在一个事务中查询语句可以看到什么，前者是能看到其他事务提交的内容，后者是做了一次查询之后，之后每次查询得到的内容都相同。

### 6.1 mvcc 多版本并发控制

**undo日志**

对于数据库中的每一行，会有两个隐藏的列，trx_id和roll_pointer，其中trx_id是最新修改当前数据的事务id，roll_pointer指向了当前列之前的版本，其实是指向了undo日志，日志中的内容和实际的数据行格式一致，里面存放的都是之前版本的数据，这样，从最新的数据行开始，就能找到这行数据的一整个历史版本链路。

![截屏2022-10-03 15.20.46](../截屏2022-10-03 15.20.46.png)

-----

**一致性视图 read-view**

当一个事务开始查询，会生成一个read-view，由当前所有已创建且未提交的事务id构成，其中未提交最小的事务id为min，已经创建的最大事务id为max，同时维护一个未提交的事务集合，基于这两个值，可以构成三个区间：

- 事务id小于min的，为已提交区间；

- 事务id大于等于min，小于等于max的，事务状态可能是提交或者未提交，之所以有已提交的事务，是因为肯定会有事务开启的晚，提交的快；

- 大于max的，为未开始的事务。

之后当读取到某一行，即找到该行的事务id，看事务id落到了哪个区：

- 如果落在了已提交区间，说明已提交，是可见的；

- 如果落在了中间区域，看未提交事务集合中是否存在该事务id，说明未提交，说明数据不可见，基于roll_pointer从undo日志中找到该行的上一个版本行，之后再次判断取到行的事务id，直到数据是可见的；

- 如果落在了未开始的事务，说明是查询之后才开始的事务，不可见。

之所以做这么三次判断，是因为小于min以及大于max都可以直接确定结果，只有落在中间区域，才要进一步判断。<font color='red'>mvcc中保证当前事务没有提交，生成的read-view不会改变，即保证同一个事务中，前后读取的数据都是一致的。</font>

------

Begin并不是一个事务的起点，做了修改操作的语句开始执行时，事务正式开始，向mysql申请事务id，mysql中的事务id是严格按照申请顺序来递增的，这样才能保证mvcc的正确性。

这里分<font color='blue'>快照读</font>和<font color='blue'>当前读</font>两个概念，快照读保证读取的结果按照给定的隔离级别展示，当前读即真实结果，是更新操作的时候操作的值。同时，read-view是针对数据库中所有的innodb的表来的，不管操作的是哪个表，大家都是按照mvcc那一套事务比对极致来的。对于每行数据的版本链，同一时刻只会有一份，但是不同的事务都有自己的read-view，每个事务针对自己的read-view结合数据版本链决定展示的内容。

在read-commited的隔离级别，每次查询语句都会生成新的read-view，之后基于read-view做比对，效果就是，在rc的隔离级别下，可以读取到其他事务提交的内容。

### 6.2 mysql buffer pool缓存机制

这里讲innodb是怎么存储数据的，mysql更新数据是直接修改内存的，之后，内存中的数据会在恰当的时机写入内存，提升io的效率，这块内存即buffer pool，下面是更新数据的流程

1. mysql按页从磁盘读取数据，之后加载到buffer pool
2. 有客户端要修改对应的数据，首先更新undo日志，之后更新内存
3. 写入redo日志缓存
4. 客户端要提交事务，将redo日志刷到硬盘
5. 写入binlog
6. 同步binlog和redo日志
7. 之后mysql在合适的时机将buffer pool的内容写入磁盘

innodb引擎中，加入了buffer pool以及redo日志的机制，客户端对数据的修改直接落脚到buffer pool中，之后在合适时间刷到硬盘，提升了io效率，**使用redo日志做保险，在系统宕机的时候用redo日志恢复buffer pool中的内容**。所以相对于直接刷硬盘，innodb把数据更新操作放到了内存中，同时在每次更新过程中增加了redo日志写入的操作，这里redo日志的操作是顺序写入的，相对于直接刷硬盘的随机读写，效率会更高，这种设计跟kafka的日志写入类似。

直接刷硬盘的操作效率肯定会有问题，因为每次要操作的数据的位置是不确定的，所以这样每次都涉及随机读写的操作，效率不会太高。
